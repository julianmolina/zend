...
ZEPHIR_INIT_CLASS(Zend_Stdlib_SplQueue) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Zend\\Stdlib, SplQueue, zend, stdlib_splqueue, spl_ce_SplQueue, zend_stdlib_splqueue_method_entry, 0);

	zend_class_implements(zend_stdlib_splqueue_ce TSRMLS_CC, 1, zend_ce_serializable);
	return SUCCESS;
...
PHP_METHOD(Zend_Stdlib_SplQueue, toArray) {
	...
	for (;_0->funcs->valid(_0 TSRMLS_CC) == SUCCESS && !EG(exception); _0->funcs->move_forward(_0 TSRMLS_CC)) {
		...
		zephir_array_append(&data, item, PH_SEPARATE, "zend/stdlib/splqueue.zep", 29);
	}
...
PHP_METHOD(Zend_Stdlib_SplQueue, unserialize) {
	...
	zephir_check_call_status();
	zephir_is_iterable(unserialized, &_2, &_1, 0, 0, "zend/stdlib/splqueue.zep", 64);
	for (
