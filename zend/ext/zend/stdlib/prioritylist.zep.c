...
ZEPHIR_INIT_CLASS(Zend_Stdlib_PriorityList) {
	...
	ZEPHIR_REGISTER_CLASS(Zend\\Stdlib, PriorityList, zend, stdlib_prioritylist, zend_stdlib_prioritylist_method_entry, 0);
	 ...
	 * @var array
	 */
	zend_declare_property_null(zend_stdlib_prioritylist_ce, SL("items"), ZEND_ACC_PROTECTED TSRMLS_CC);
	 ...
	 * @var int
	 */
	zend_declare_property_long(zend_stdlib_prioritylist_ce, SL("serial"), 0, ZEND_ACC_PROTECTED TSRMLS_CC);

	/**
	 * Serial order mode
	 * @var integer
	 */
	zend_declare_property_long(zend_stdlib_prioritylist_ce, SL("isLIFO"), 1, ZEND_ACC_PROTECTED TSRMLS_CC);
	 ...
	 * @var int
	 */
	zend_declare_property_long(zend_stdlib_prioritylist_ce, SL("count"), 0, ZEND_ACC_PROTECTED TSRMLS_CC);
	 ...
	 * @var bool
	 */
	zend_declare_property_bool(zend_stdlib_prioritylist_ce, SL("sorted"), 0, ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_class_constant_long(zend_stdlib_prioritylist_ce, SL("EXTR_DATA"), 0x00000001 TSRMLS_CC);

	zend_declare_class_constant_long(zend_stdlib_prioritylist_ce, SL("EXTR_PRIORITY"), 0x00000002 TSRMLS_CC);

	zend_declare_class_constant_long(zend_stdlib_prioritylist_ce, SL("EXTR_BOTH"), 0x00000003 TSRMLS_CC);

	zend_class_implements(zend_stdlib_prioritylist_ce TSRMLS_CC, 1, zend_ce_iterator);
	zend_class_implements(zend_stdlib_prioritylist_ce TSRMLS_CC, 1, spl_ce_Countable);
	return SUCCESS;
...
PHP_METHOD(Zend_Stdlib_PriorityList, setPriority) {
	...
	if (unlikely(!zephir_array_isset(_0, name))) {
		...
		ZEPHIR_INIT_VAR(_1);
		object_init_ex(_1, zend_stdlib_exception_invalidargumentexception_ce);
		ZEPHIR_CALL_METHOD(NULL, _1, "__construct", NULL, exceptionMsg);
		zephir_check_call_status();
		zephir_throw_exception_debug(_1, "zend/stdlib/prioritylist.zep", 87 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_INIT_NVAR(_1);
...
PHP_METHOD(Zend_Stdlib_PriorityList, compare) {
	...
	zephir_array_fetch_string(&_0, item1, SL("priority"), PH_NOISY | PH_READONLY, "zend/stdlib/prioritylist.zep", 184 TSRMLS_CC);
	zephir_array_fetch_string(&_1, item2, SL("priority"), PH_NOISY | PH_READONLY, "zend/stdlib/prioritylist.zep", 184 TSRMLS_CC);
	if (ZEPHIR_IS_IDENTICAL(_0, _1)) {
		zephir_array_fetch_string(&_2, item1, SL("serial"), PH_NOISY | PH_READONLY, "zend/stdlib/prioritylist.zep", 185 TSRMLS_CC);
		zephir_array_fetch_string(&_3, item2, SL("serial"), PH_NOISY | PH_READONLY, "zend/stdlib/prioritylist.zep", 185 TSRMLS_CC);
		if (ZEPHIR_GT(_2, _3)) {
			result = -1;
		} else {
			result = 1;
		...
		zephir_array_fetch_string(&_2, item1, SL("priority"), PH_NOISY | PH_READONLY, "zend/stdlib/prioritylist.zep", 192 TSRMLS_CC);
		zephir_array_fetch_string(&_3, item2, SL("priority"), PH_NOISY | PH_READONLY, "zend/stdlib/prioritylist.zep", 192 TSRMLS_CC);
		if (ZEPHIR_GT(_2, _3)) {
			result = -1;
		} else {
			result = 1;
...
PHP_METHOD(Zend_Stdlib_PriorityList, current) {
	...
	if (!ZEPHIR_IS_FALSE_IDENTICAL(node)) {
		zephir_array_fetch_string(&_2, node, SL("data"), PH_NOISY | PH_READONLY, "zend/stdlib/prioritylist.zep", 246 TSRMLS_CC);
		RETURN_CTOR(_2);
	}
	RETURN_MM_BOOL(0);
...
PHP_METHOD(Zend_Stdlib_PriorityList, next) {
	...
	zephir_check_call_status();
	if (!ZEPHIR_IS_FALSE_IDENTICAL(node)) {
		zephir_array_fetch_string(&_2, node, SL("data"), PH_NOISY | PH_READONLY, "zend/stdlib/prioritylist.zep", 274 TSRMLS_CC);
		RETURN_CTOR(_2);
	}
	RETURN_MM_BOOL(0);
