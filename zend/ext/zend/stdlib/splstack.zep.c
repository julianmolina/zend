...
ZEPHIR_INIT_CLASS(Zend_Stdlib_SplStack) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Zend\\Stdlib, SplStack, zend, stdlib_splstack, spl_ce_SplStack, zend_stdlib_splstack_method_entry, 0);

	zend_class_implements(zend_stdlib_splstack_ce TSRMLS_CC, 1, zend_ce_serializable);
	return SUCCESS;
...
PHP_METHOD(Zend_Stdlib_SplStack, toArray) {
	...
	for (;_0->funcs->valid(_0 TSRMLS_CC) == SUCCESS && !EG(exception); _0->funcs->move_forward(_0 TSRMLS_CC)) {
		...
		zephir_array_append(&data, item, PH_SEPARATE, "zend/stdlib/splstack.zep", 27);
	}
...
PHP_METHOD(Zend_Stdlib_SplStack, unserialize) {
	...
	zephir_check_call_status();
	zephir_is_iterable(unserialized, &_2, &_1, 0, 0, "zend/stdlib/splstack.zep", 61);
	for (
	  ; zephir_hash_get_current_data_ex(_2, (void**) &_3, &_1) == SUCCESS
	  ; zephir_hash_move_forward_ex(_2, &_1)
	) {
		ZEPHIR_GET_HVALUE(item, _3);
		ZEPHIR_CALL_METHOD(NULL, this_ptr, "unshift", &_4, item);
		zephir_check_call_status();
	}
	ZEPHIR_MM_RESTORE();
