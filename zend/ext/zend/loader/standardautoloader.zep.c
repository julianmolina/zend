...
ZEPHIR_INIT_CLASS(Zend_Loader_StandardAutoloader) {
	...
	ZEPHIR_REGISTER_CLASS(Zend\\Loader, StandardAutoloader, zend, loader_standardautoloader, zend_loader_standardautoloader_method_entry, 0);

	/**
	 * @var array Namespace/directory pairs to search; ZF library added by default
	 */
	zend_declare_property_null(zend_loader_standardautoloader_ce, SL("namespaces"), ZEND_ACC_PROTECTED TSRMLS_CC);

	/**
	 * @var array Prefix/directory pairs to search
	 */
	zend_declare_property_null(zend_loader_standardautoloader_ce, SL("prefixes"), ZEND_ACC_PROTECTED TSRMLS_CC);

	/**
	 * @var bool Whether or not the autoloader should also act as a fallback autoloader
	 */
	zend_declare_property_bool(zend_loader_standardautoloader_ce, SL("fallbackAutoloaderFlag"), 0, ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_class_constant_string(zend_loader_standardautoloader_ce, SL("NS_SEPARATOR"), "\\" TSRMLS_CC);

	zend_declare_class_constant_string(zend_loader_standardautoloader_ce, SL("PREFIX_SEPARATOR"), "_" TSRMLS_CC);

	zend_declare_class_constant_string(zend_loader_standardautoloader_ce, SL("LOAD_NS"), "namespaces" TSRMLS_CC);

	zend_declare_class_constant_string(zend_loader_standardautoloader_ce, SL("LOAD_PREFIX"), "prefixes" TSRMLS_CC);

	zend_declare_class_constant_string(zend_loader_standardautoloader_ce, SL("ACT_AS_FALLBACK"), "fallback_autoloader" TSRMLS_CC);

	zend_declare_class_constant_string(zend_loader_standardautoloader_ce, SL("AUTOREGISTER_ZF"), "autoregister_zf" TSRMLS_CC);

	zend_class_implements(zend_loader_standardautoloader_ce TSRMLS_CC, 1, zend_loader_splautoloader_ce);
	return SUCCESS;
...
PHP_METHOD(Zend_Loader_StandardAutoloader, setOptions) {
	...
	if (Z_TYPE_P(options) != IS_ARRAY) {
		...
		if (unlikely(!zephir_is_true(_1))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(zend_loader_exception_invalidargumentexception_ce, "Options must be either an array or Traversable", "zend/loader/standardautoloader.zep", 92);
			return;
		}
	...
	zephir_is_iterable(options, &_5, &_4, 0, 0, "zend/loader/standardautoloader.zep", 121);
	for (
	  ; zephir_hash_get_current_data_ex(_5, (void**) &_6, &_4) == SUCCESS
	  ; zephir_hash_move_forward_ex(_5, &_4)
	) {
		ZEPHIR_GET_HMKEY(type, _5, _4);
...
PHP_METHOD(Zend_Loader_StandardAutoloader, registerNamespaces) {
	...
	if (Z_TYPE_P(namespaces) != IS_ARRAY) {
		...
		zephir_check_call_status();
		if (unlikely(!zephir_is_true(_1))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(zend_loader_exception_invalidargumentexception_ce, "Namespace pairs must be either an array or Traversable", "zend/loader/standardautoloader.zep", 180);
			return;
		}
	...
	zephir_is_iterable(namespaces, &_5, &_4, 0, 0, "zend/loader/standardautoloader.zep", 187);
	for (
	  ; zephir_hash_get_current_data_ex(_5, (void**) &_6, &_4) == SUCCESS
	  ; zephir_hash_move_forward_ex(_5, &_4)
	) {
...
PHP_METHOD(Zend_Loader_StandardAutoloader, registerPrefixes) {
	...
	if (Z_TYPE_P(prefixes) != IS_ARRAY) {
		...
		zephir_check_call_status();
		if (unlikely(!zephir_is_true(_1))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(zend_loader_exception_invalidargumentexception_ce, "Prefix pairs must be either an array or Traversable", "zend/loader/standardautoloader.zep", 223);
			return;
		}
	...
	zephir_is_iterable(prefixes, &_5, &_4, 0, 0, "zend/loader/standardautoloader.zep", 230);
	for (
	  ; zephir_hash_get_current_data_ex(_5, (void**) &_6, &_4) == SUCCESS
	  ; zephir_hash_move_forward_ex(_5, &_4)
	) {
...
PHP_METHOD(Zend_Loader_StandardAutoloader, loadClass) {
	...
	if (unlikely(!zephir_fast_in_array(type, _0 TSRMLS_CC))) {
		ZEPHIR_INIT_NVAR(_1);
		object_init_ex(_1, zend_loader_exception_invalidargumentexception_ce);
		ZEPHIR_CALL_METHOD(NULL, _1, "__construct", NULL);
		zephir_check_call_status();
		zephir_throw_exception_debug(_1, "zend/loader/standardautoloader.zep", 326 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	...
	zephir_is_iterable(paths, &_4, &_3, 0, 0, "zend/loader/standardautoloader.zep", 367);
	for (
