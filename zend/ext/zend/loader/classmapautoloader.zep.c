...
ZEPHIR_INIT_CLASS(Zend_Loader_ClassMapAutoloader) {
	...
	ZEPHIR_REGISTER_CLASS(Zend\\Loader, ClassMapAutoloader, zend, loader_classmapautoloader, zend_loader_classmapautoloader_method_entry, 0);

	/**
	 * Registry of map files that have already been loaded
	 * @var array
	 */
	zend_declare_property_null(zend_loader_classmapautoloader_ce, SL("mapsLoaded"), ZEND_ACC_PROTECTED TSRMLS_CC);

	/**
	 * Class name/filename map
	 * @var array
	 */
	zend_declare_property_null(zend_loader_classmapautoloader_ce, SL("map"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_class_implements(zend_loader_classmapautoloader_ce TSRMLS_CC, 1, zend_loader_splautoloader_ce);
	return SUCCESS;
...
PHP_METHOD(Zend_Loader_ClassMapAutoloader, registerAutoloadMap) {
	...
	if (unlikely(Z_TYPE_P(map) != IS_ARRAY)) {
		...
		ZEPHIR_INIT_NVAR(_0);
		object_init_ex(_0, zend_loader_exception_invalidargumentexception_ce);
		ZEPHIR_CALL_METHOD(NULL, _0, "__construct", NULL, exceptionMsg);
		zephir_check_call_status();
		zephir_throw_exception_debug(_0, "zend/loader/classmapautoloader.zep", 100 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	...
	if (!(ZEPHIR_IS_EMPTY(location))) {
		...
		zephir_array_append(&mapsLoaded, location, PH_SEPARATE, "zend/loader/classmapautoloader.zep", 114);
...
PHP_METHOD(Zend_Loader_ClassMapAutoloader, registerAutoloadMaps) {
	...
	if (Z_TYPE_P(locations) != IS_ARRAY) {
		...
		if (unlikely(!zephir_is_true(_1))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(zend_loader_exception_invalidargumentexception_ce, "Map list must be an array or implement Traversable", "zend/loader/classmapautoloader.zep", 133);
			return;
		}
	...
	zephir_is_iterable(locations, &_5, &_4, 0, 0, "zend/loader/classmapautoloader.zep", 141);
	for (
...
PHP_METHOD(Zend_Loader_ClassMapAutoloader, loadMapFromFile) {
	...
	if (unlikely(!(zephir_file_exists(location TSRMLS_CC) == SUCCESS))) {
		...
		ZEPHIR_CALL_METHOD(NULL, _0, "__construct", NULL, exceptionMsg);
		zephir_check_call_status();
		zephir_throw_exception_debug(_0, "zend/loader/classmapautoloader.zep", 205 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(Zend_Loader_ClassMapAutoloader, realPharPath) {
	...
	zephir_array_fetch_long(&_3, match, 1, PH_NOISY | PH_READONLY, "zend/loader/classmapautoloader.zep", 242 TSRMLS_CC);
	...
	ZEPHIR_CPY_WRT(parts, _0);
	zephir_is_iterable(parts, &_9, &_8, 0, 0, "zend/loader/classmapautoloader.zep", 251);
	for (
	  ...
	  ; zephir_hash_move_forward_ex(_9, &_8)
		...
		if (_11) {
			zephir_array_append(&partsFiltered, value, PH_SEPARATE, "zend/loader/classmapautoloader.zep", 248);
		}
	}
	ZEPHIR_CALL_FUNCTION(&parts, "array_values", &_12, partsFiltered);
	zephir_check_call_status();
	zephir_is_iterable(parts, &_14, &_13, 1, 0, "zend/loader/classmapautoloader.zep", 260);
	for (
	  ; zephir_hash_get_current_data_ex(_14, (void**) &_15, &_13) == SUCCESS
	  ; zephir_hash_move_forward_ex(_14, &_13)
	) {
		ZEPHIR_GET_HMKEY(key, _14, _13);
