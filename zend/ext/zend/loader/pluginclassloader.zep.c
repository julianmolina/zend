...
ZEPHIR_INIT_CLASS(Zend_Loader_PluginClassLoader) {
	...
	ZEPHIR_REGISTER_CLASS(Zend\\Loader, PluginClassLoader, zend, loader_pluginclassloader, zend_loader_pluginclassloader_method_entry, 0);

	/**
	 * List of plugin name => class name pairs
	 * @var array
	 */
	zend_declare_property_null(zend_loader_pluginclassloader_ce, SL("plugins"), ZEND_ACC_PROTECTED TSRMLS_CC);

	/**
	 * Static map allow global seeding of plugin loader
	 * @var array
	 */
	zend_declare_property_null(zend_loader_pluginclassloader_ce, SL("staticMap"), ZEND_ACC_PROTECTED|ZEND_ACC_STATIC TSRMLS_CC);

	zend_class_implements(zend_loader_pluginclassloader_ce TSRMLS_CC, 1, zend_loader_pluginclasslocator_ce);
	return SUCCESS;
...
PHP_METHOD(Zend_Loader_PluginClassLoader, addStaticMap) {
	...
	if (Z_TYPE_P(map) != IS_ARRAY) {
		...
		if (unlikely(!zephir_is_true(_1))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(zend_loader_exception_invalidargumentexception_ce, "Expects an array or Traversable object", "zend/loader/pluginclassloader.zep", 83);
			return;
		}
	...
	zephir_is_iterable(map, &_5, &_4, 0, 0, "zend/loader/pluginclassloader.zep", 98);
	for (
...
PHP_METHOD(Zend_Loader_PluginClassLoader, registerPlugins) {
	...
	if (Z_TYPE_P(map) == IS_STRING) {
		if (unlikely(!zephir_class_exists(map, 1 TSRMLS_CC))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(zend_loader_exception_invalidargumentexception_ce, "Map class provided is invalid", "zend/loader/pluginclassloader.zep", 146);
			return;
		}
	...
	if (unlikely(!zephir_is_true(_3))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(zend_loader_exception_invalidargumentexception_ce, "Map provided is invalid; must be traversable", "zend/loader/pluginclassloader.zep", 156);
		return;
	}
