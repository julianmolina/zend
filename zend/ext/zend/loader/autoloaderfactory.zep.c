...
ZEPHIR_INIT_CLASS(Zend_Loader_AutoloaderFactory) {
	...
	ZEPHIR_REGISTER_CLASS(Zend\\Loader, AutoloaderFactory, zend, loader_autoloaderfactory, zend_loader_autoloaderfactory_method_entry, ZEND_ACC_EXPLICIT_ABSTRACT_CLASS);

	/**
	 * @var array All autoloaders registered using the factory
	 */
	zend_declare_property_null(zend_loader_autoloaderfactory_ce, SL("loaders"), ZEND_ACC_PROTECTED|ZEND_ACC_STATIC TSRMLS_CC);

	/**
	 * @var StandardAutoloader StandardAutoloader instance for resolving
	 * autoloader classes via the include_path
	 */
	zend_declare_property_null(zend_loader_autoloaderfactory_ce, SL("standardAutoloader"), ZEND_ACC_PROTECTED|ZEND_ACC_STATIC TSRMLS_CC);

	zend_declare_class_constant_string(zend_loader_autoloaderfactory_ce, SL("STANDARD_AUTOLOADER"), "Zend\\Loader\\StandardAutoloader" TSRMLS_CC);
...
PHP_METHOD(Zend_Loader_AutoloaderFactory, factory) {
	...
	if (Z_TYPE_P(options) != IS_ARRAY) {
		...
		if (unlikely(!zephir_is_true(_3))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(zend_loader_exception_invalidargumentexception_ce, "Options provided must be an array or Traversable", "zend/loader/autoloaderfactory.zep", 90);
			return;
		}
	...
	zephir_is_iterable(options, &_7, &_6, 0, 0, "zend/loader/autoloaderfactory.zep", 132);
	for (
	  ...
	  ; zephir_hash_move_forward_ex(_7, &_6)
		...
		if (unlikely(_10)) {
			...
			object_init_ex(_11, zend_loader_exception_invalidargumentexception_ce);
			ZEPHIR_CALL_METHOD(NULL, _11, "__construct", &_12, exceptionMsg);
			zephir_check_call_status();
			zephir_throw_exception_debug(_11, "zend/loader/autoloaderfactory.zep", 110 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
		...
		if (unlikely(!zephir_is_true(_5))) {
			...
			ZEPHIR_CALL_METHOD(NULL, _11, "__construct", &_12, exceptionMsg);
			zephir_check_call_status();
			zephir_throw_exception_debug(_11, "zend/loader/autoloaderfactory.zep", 115 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
		ZEPHIR_INIT_LNVAR(_13);
...
PHP_METHOD(Zend_Loader_AutoloaderFactory, getRegisteredAutoloader) {
	...
	zephir_check_call_status();
	zephir_throw_exception_debug(_1, "zend/loader/autoloaderfactory.zep", 182 TSRMLS_CC);
	ZEPHIR_MM_RESTORE();
	return;
...
PHP_METHOD(Zend_Loader_AutoloaderFactory, unregisterAutoloaders) {
	...
	ZEPHIR_INIT_VAR(className);
	zephir_get_called_class(className TSRMLS_CC);
	zephir_is_iterable(autoloaders, &_1, &_0, 0, 0, "zend/loader/autoloaderfactory.zep", 210);
	for (
	  ; zephir_hash_get_current_data_ex(_1, (void**) &_2, &_0) == SUCCESS
	  ; zephir_hash_move_forward_ex(_1, &_0)
	) {
		ZEPHIR_GET_HMKEY(autoloaderClass, _1, _0);
