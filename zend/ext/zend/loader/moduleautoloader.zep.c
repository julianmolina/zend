...
ZEPHIR_INIT_CLASS(Zend_Loader_ModuleAutoloader) {
	...
	ZEPHIR_REGISTER_CLASS(Zend\\Loader, ModuleAutoloader, zend, loader_moduleautoloader, zend_loader_moduleautoloader_method_entry, 0);

	/**
	 * @var array An array of module paths to scan
	 */
	zend_declare_property_null(zend_loader_moduleautoloader_ce, SL("paths"), ZEND_ACC_PROTECTED TSRMLS_CC);

	/**
	 * @var array An array of modulename => path
	 */
	zend_declare_property_null(zend_loader_moduleautoloader_ce, SL("explicitPaths"), ZEND_ACC_PROTECTED TSRMLS_CC);

	/**
	 * @var array An array of namespaceName => namespacePath
	 */
	zend_declare_property_null(zend_loader_moduleautoloader_ce, SL("namespacedPaths"), ZEND_ACC_PROTECTED TSRMLS_CC);

	/**
	 * @var array An array of supported phar extensions (filled on constructor)
	 */
	zend_declare_property_null(zend_loader_moduleautoloader_ce, SL("pharExtensions"), ZEND_ACC_PROTECTED TSRMLS_CC);

	/**
	 * @var array An array of module classes to their containing files
	 */
	zend_declare_property_null(zend_loader_moduleautoloader_ce, SL("moduleClassMap"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_class_implements(zend_loader_moduleautoloader_ce TSRMLS_CC, 1, zend_loader_splautoloader_ce);
	return SUCCESS;
...
PHP_METHOD(Zend_Loader_ModuleAutoloader, autoload) {
	...
	if (zephir_fast_count_int(namespacedPaths TSRMLS_CC) >= 1) {
		zephir_is_iterable(namespacedPaths, &_7, &_6, 0, 0, "zend/loader/moduleautoloader.zep", 193);
		for (
	...
	zephir_is_iterable(paths, &_19, &_18, 0, 0, "zend/loader/moduleautoloader.zep", 248);
	for (
...
PHP_METHOD(Zend_Loader_ModuleAutoloader, registerPaths) {
	...
	if (Z_TYPE_P(paths) != IS_ARRAY) {
		...
		if (unlikely(!zephir_is_true(_1))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(zend_loader_exception_invalidargumentexception_ce, "Parameter to \\Zend\\Loader\\ModuleAutoloader's registerPaths method must be an array or implement the Traversable interface", "zend/loader/moduleautoloader.zep", 378);
			return;
		}
	...
	zephir_is_iterable(paths, &_5, &_4, 0, 0, "zend/loader/moduleautoloader.zep", 391);
	for (
...
PHP_METHOD(Zend_Loader_ModuleAutoloader, registerPath) {
	...
	if (unlikely(Z_TYPE_P(path) != IS_STRING)) {
		...
		ZEPHIR_INIT_VAR(_2);
		object_init_ex(_2, zend_loader_exception_invalidargumentexception_ce);
		ZEPHIR_CALL_METHOD(NULL, _2, "__construct", NULL, exceptionMsg);
		zephir_check_call_status();
		zephir_throw_exception_debug(_2, "zend/loader/moduleautoloader.zep", 409 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(Zend_Loader_ModuleAutoloader, pharFileToModuleName) {
	...
	do {
		...
		zephir_array_fetch_string(&path, info, SL("filename"), PH_NOISY, "zend/loader/moduleautoloader.zep", 454 TSRMLS_CC);
	} while (zephir_array_isset_string(info, SS("extension")));
	zephir_array_fetch_string(&_1, info, SL("filename"), PH_NOISY | PH_READONLY, "zend/loader/moduleautoloader.zep", 457 TSRMLS_CC);
	RETURN_CTOR(_1);
