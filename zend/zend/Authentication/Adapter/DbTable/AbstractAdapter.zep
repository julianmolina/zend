...
abstract class AbstractAdapter extends BaseAdapter
{
    ...
    public function getResultRowObject(returnColumns = null, omitColumns = null)
    {
        ...
        if null !== returnColumns {
            ...
            let availableColumns = array_keys(this->resultRow);
            for  returnColumn in returnColumns {
                if in_array(returnColumn, availableColumns) {
                    let returnObject->{returnColumn} = this->resultRow[returnColumn];
                }
            }
            return returnObject;
            ...
            let omitColumns = omitColumns;
            for  resultColumn, resultValue in this->resultRow {
                if !in_array(resultColumn, omitColumns) {
                    let returnObject->{resultColumn} = resultValue;
                }
            }
            return returnObject;
    ...
    public function authenticate()
    {
        ...
        let dbSelect         = this->authenticateCreateSelect();
        let resultIdentities = this->authenticateQuerySelect(dbSelect);
        ...
        for identity in resultIdentities {
            let authResult = this->authenticateValidateResult(identity);
    ...
    protected function authenticateSetup()
    {
        ...
        let this->authenticateResultInfo = [
    ...
    {
        let sql = new Sql\Sql(this->zendDb);
        let statement = sql->prepareStatementForSqlObject(dbSelect);
        try {
            let result = statement->execute();
            let resultIdentities = [];
            // iterate result, most cross platform way
            for row in result {
                let resultIdentities[] = row;
            }
        } catch \Exception e {
    ...
    protected function authenticateValidateResultSet(array resultIdentities)
    {
        ...
        if count(resultIdentities) < 1 {
            let this->authenticateResultInfo['code']       = AuthenticationResult::FAILURE_IDENTITY_NOT_FOUND;
            let this->authenticateResultInfo['messages'][] = 'A record with the supplied identity could not be found.';
            return this->authenticateCreateAuthResult();
        } elseif count(resultIdentities) > 1 && false === this->getAmbiguityIdentity() {
            let this->authenticateResultInfo['code']       = AuthenticationResult::FAILURE_IDENTITY_AMBIGUOUS;
            let this->authenticateResultInfo['messages'][] = 'More than one record matches the supplied identity.';
            return this->authenticateCreateAuthResult();
        }
