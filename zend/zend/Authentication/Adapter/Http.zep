...
class Http implements AdapterInterface
{
    ...
    public function __construct(array config)
    {
        ...
        let this->response = null;
        let this->ieNoOpaque = false;

        if empty(config['accept_schemes']) {
            throw new Exception\InvalidArgumentException('Config key "accept_schemes" is required');
        }

        let schemes = explode(' ', config['accept_schemes']);
        let this->acceptSchemes = array_intersect(schemes, this->supportedSchemes);
        ...
        if in_array('digest', this->acceptSchemes) {
            if empty(config['digest_domains']) ||
                ...
                let this->domains = config['digest_domains'];
            }
        ...
        if isset(config['proxy_auth']) && true == (bool) config['proxy_auth'] {
            let this->imaProxy = true;  // I'm a Proxy
        } else {
            let this->imaProxy = false;
        }
    }
    ...
    {
        let this->basicResolver = resolver;
    ...
    {
        let this->digestResolver = resolver;
    ...
    public function authenticate()
    {
        ...
        if this->imaProxy {
            let getHeader = 'Proxy-Authorization';
        } else {
            let getHeader = 'Authorization';
        }
        ...
        let authHeader = headers->get(getHeader)->getFieldValue();
        if !authHeader {
            return this->_challengeClient();
        }

        let list(clientScheme) = explode(' ', authHeader);
        let clientScheme = strtolower(clientScheme);
        ...
        switch clientScheme {
            case 'basic':
                    let result = this->_basicAuth(authHeader);
                break;
            case 'digest':
                    let result = this->_digestAuth(authHeader);
                break;
            default:
    ...
    protected function _challengeClient()
    {
        if this->imaProxy {
            let statusCode = 407;
            let headerName = 'Proxy-Authenticate';
        } else {
            let statusCode = 401;
        ...
        let headers = this->response->getHeaders();
    ...
    protected function _basicAuth(header)
    {
        ...
        let auth = substr(header, strlen('Basic '));
        let auth = base64_decode(auth);
        if !auth {
        ...
        let creds = array_filter(explode(':', auth));
        if count(creds) != 2 {
            return this->_challengeClient();
        }

        let result = this->basicResolver->resolve(creds[0], this->realm, creds[1]);
        ...
        {
            let identity = ['username' : creds[0], 'realm' : this->realm];
    ...
    protected function _digestAuth(header)
    {
        ...
        let data = this->_parseDigestAuth(header);
        if data === false {
            this->response->setStatusCode(400);
            return new Authentication\Result(
                Authentication\Result::FAILURE_UNCATEGORIZED,
        ...
        let ha1 = this->digestResolver->resolve(data['username'], data['realm']);
        if ha1 === false {
            return this->_challengeClient();
        }
        ...
        if this->algo == 'MD5-sess' {
            let ha1 = hash('md5', ha1 . ':' . data['nonce'] . ':' . data['cnonce']);
        }
        ...
        switch data['qop'] {
            case 'auth':
                let a2 = this->request->getMethod() . ':' . data['uri'];
                break;
            case 'auth-int':
        ...
        let message = data['nonce'] . ':' . data['nc'] . ':' . data['cnonce'] . ':' . data['qop'] . ':' . ha2;
        let digest  = hash('md5', ha1 . ':' . message);

        // If our digest matches the client's let them in, otherwise return
        // a 401 code and exit to prevent access to the protected resource.
        if CryptUtils::compareStrings(digest, data['response']) {
            let identity = ['username' : data['username'], 'realm' : data['realm']];
            return new Authentication\Result(Authentication\Result::SUCCESS, identity);
        }
    ...
    protected function _calcNonce()
    {
        ...
        let timeout = ceil(time() / this->nonceTimeout) * this->nonceTimeout;

        let userAgentHeader = this->request->getHeaders()->get('User-Agent');
        if userAgentHeader {
            userAgent = userAgentHeader->getFieldValue();
        } elseif isset(_SERVER['HTTP_USER_AGENT']) {
            let userAgent = _SERVER['HTTP_USER_AGENT'];
        } else {
            let userAgent = 'Zend_Authenticaion';
        }
        let nonce = hash('md5', timeout . ':' . userAgent . ':' . __CLASS__);
        return nonce;
    }
    ...
    protected function _parseDigestAuth(header)
    {
        ...
        // 400 code.
        let ret = preg_match('/username="([^"]+)"/', header, temp);
        if !ret || empty(temp[1])
                || !ctype_print(temp[1])
                || strpos(temp[1], ':') !== false
        {
            let data['username'] = '::invalid::';
        } else {
            let data['username'] = temp[1];
        }
        let temp = null;

        let ret = preg_match('/realm="([^"]+)"/', header, temp);
        if !ret || empty(temp[1]) {
            return false;
        }
        if !ctype_print(temp[1]) || strpos(temp[1], ':') !== false {
            ...
            let data['realm'] = temp[1];
        }
        let temp = null;

        let ret = preg_match('/nonce="([^"]+)"/', header, temp);
        if !ret || empty(temp[1]) {
            return false;
        }
        ...
        let data['nonce'] = temp[1];
        let temp = null;

        let ret = preg_match('/uri="([^"]+)"/', header, temp);
        if !ret || empty(temp[1]) {
            return false;
        }
        ...
        let rUri = this->request->getUri();
        let cUri = UriFactory::factory(temp[1]);
        ...
        let ret = preg_match('/response="([^"]+)"/', header, temp);
        if !ret || empty(temp[1]) {
            return false;
        }
        ...
        let ret = preg_match('/algorithm="?(' . this->algo . ')"?/', header, temp);
        if ret && !empty(temp[1])
               && in_array(temp[1], this->supportedAlgos)
        {
            let data['algorithm'] = temp[1];
        } else {
            let data['algorithm'] = 'MD5';  // = this->algo; ?
        }
        let temp = null;

        // Not optional in this implementation
        let ret = preg_match('/cnonce="([^"]+)"/', header, temp);
        if !ret || empty(temp[1]) {
            return false;
        }
        ...
        if this->useOpaque {
            ...
            if !ret || empty(temp[1]) {
                ...
                let userAgent = headers->get('User-Agent')->getFieldValue();
                if false === strpos(userAgent, 'MSIE') {
                    return false;
                }

                let temp[1] = '';
                let this->ieNoOpaque = true;
            }
            ...
            let data['opaque'] = temp[1];
            let temp = null;
        }

        // Not optional in this implementation, but must be one of the supported
        // qop types
        let ret = preg_match('/qop="?(' . implode('|', this->supportedQops) . ')"?/', header, temp);
        if (!ret || empty(temp[1])) {
            return false;
        }
        ...
        let ret = preg_match('/nc="?([0-9A-Fa-f]{8})"?/', header, temp);
        if !ret || empty(temp[1]) {
            return false;
        }
