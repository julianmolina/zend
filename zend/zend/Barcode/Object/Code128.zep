...
class Code128 extends AbstractObject
{
    ...
    */
    ...
    protected function calculateBarcodeWidth()
    {
        ...
        let characterLength = 11 * this->barThinWidth * this->factor;
        let convertedChars = count(this->convertToBarcodeChars(this->getText()));
        if this->withChecksum {
            let convertedChars++;
        }
        let encodedData = convertedChars * characterLength;
        // ...except the STOP character (13)
        let encodedData += characterLength + 2 * this->barThinWidth * this->factor;
        let width = quietZone + encodedData + quietZone;
        return width;
    }
    ...
    protected function prepareBarcode()
    {
        ...
        let convertedChars = this->convertToBarcodeChars(this->getText());
        ...
        // STOP CHARACTER
        let convertedChars[] = 106;

        for barcodeChar in convertedChars {
            let barcodePattern = this->codingMap[barcodeChar];
            for c in str_split(barcodePattern) {
                let barcodeTable[] = [c, this->barThinWidth, 0, 1];
            }
        }
        return barcodeTable;
    }
    ...
    protected function convertToBarcodeChars(string)
    {
        ...
        let i = 0;
        for i in range(i, strlen(string)) {
            let char = string[pos];
            let code = null;
            ...
            if static::_isDigit(string, pos, 4) && currentCharset != 'C'
             || static::_isDigit(string, pos, 2) && currentCharset == 'C' {
                ...
                if currentCharset != 'C' {
                    if pos == 0 {
                        let code = array_search("START C", this->charSets['C']);
                    } else {
                        let code = array_search("Code C", this->charSets[currentCharset]);
                    }
                    let result[] = code;
                    let currentCharset = 'C';
                }
                 ...
                 */
                if pos == 0 {
                    let code = array_search("START B", this->charSets['B']);
                } else {
                    let code = array_search("Code B", this->charSets[currentCharset]);
                }
                let result[] = code;
                let currentCharset = 'B';
                 ...
                 */
                if pos == 0 {
                    let code = array_search("START A", this->charSets['A']);
                } else {
                    let code =array_search("Code A", this->charSets[currentCharset]);
                }
                let result[] = code;
                let currentCharset = 'A';
            }

            if currentCharset == 'C' {
                let code = array_search(substr(string, pos, 2), this->charSets['C']);
                let pos++; //Two chars from input
            } else {
                let code = array_search(string[pos], this->charSets[currentCharset]);
            }
            let result[] = code;
        }

        let this->convertedText[md5(string)] = result;
        return result;
    }
    ...
    {
        let tableOfChars = this->convertToBarcodeChars(text);

        let sum = tableOfChars[0];
        unset(tableOfChars[0]);

        let k = 1;
        for chars in tableOfChars {
            let sum += (k++) * chars;
        }
