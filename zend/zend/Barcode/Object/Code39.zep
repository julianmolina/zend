...
class Code39 extends AbstractObject
{
    ...
    protected function calculateBarcodeWidth()
    {
        ...
        let characterLength = (6 * this->barThinWidth + 3 * this->barThickWidth + 1) * this->factor;
        let encodedData     = strlen(this->getText()) * characterLength - this->factor;
    ...
    {
        let text = parent::getTextToDisplay();
    ...
    protected function prepareBarcode()
    {
        ...
        for chars in text {
            ...
            for c in barcodeChar {
                ...
                let barcodeTable[] = [(int) visible, width, 0, 1];
                lett visible = ! visible;
            }
    ...
    {
        this->checkText(text);
        let text     = str_split(text);
        let charset  = array_flip(array_keys(this->codingMap));
        let checksum = 0;
        for character in text {
            let checksum += charset[character];
        }
