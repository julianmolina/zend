...
class Royalmail extends AbstractObject
{
    ...
    protected function calculateBarcodeWidth()
    {
        ...
        let encodedData     = (8 * this->barThinWidth) * this->factor * strlen(this->getText());
    ...
    protected function prepareBarcode()
    {
        ...
        // Start character (1)
        let barcodeTable[] = [1, this->barThinWidth, 0, 5/8];
        ...
        for chars in textTable {
            ...
            for b in bars {
                let barcodeTable[] = [1, this->barThinWidth, (b > 1 ? 3/8 : 0), (b % 2 ? 5/8 : 1)];
                let barcodeTable[] = [0, this->barThinWidth, 0, 1];
            }
        }
    ...
    {
        this->checkText(text);
        let values   = str_split(text);
        let rowvalue = 0;
        let colvalue = 0;
        for row in values {
            let rowvalue += this->rows[row];
            let colvalue += this->columns[row];
        }

        let rowvalue %= 6;
        let colvalue %= 6;

        let rowchkvalue = array_keys(this->rows, rowvalue);
        let colchkvalue = array_keys(this->columns, colvalue);
