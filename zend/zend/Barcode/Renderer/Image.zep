...
class Image extends AbstractRenderer
{
     ...
     * @var array
     */
    protected allowedImageType = ['png', 'jpeg', 'gif'];
    ...
    protected function initRenderer()
    {
        ...
        if this->resource !== null {
            ...
            let foreColor       = this->barcode->getForeColor();
            let backgroundColor = this->barcode->getBackgroundColor();
            let this->imageBackgroundColor = imagecolorallocate(
                this->resource,
                (backgroundColor & 0xFF0000) >> 16,
                (backgroundColor & 0x00FF00) >> 8,
                backgroundColor & 0x0000FF
            );
            let this->imageForeColor = imagecolorallocate(
                this->resource,
            ...
            let foreColor       = this->barcode->getForeColor();
            let backgroundColor = this->barcode->getBackgroundColor();
            let this->resource = imagecreatetruecolor(width, height);
            ...
            let white = imagecolorallocate(this->resource, 255, 255, 255);
    ...
    protected function checkDimensions()
    {
        ...
        if this->resource !== null {
            ...
            if this->userWidth {
                let width = this->barcode->getWidth(true);
    ...
    public function render()
    {
        ...
        let functionName = 'image' . this->imageType;
    ...
    {
        let newPoints = [points[0][0] + this->leftOffset,
        ...
        let allocatedColor = imagecolorallocate(this->resource,
                                             (color & 0xFF0000) >> 16,
                                             (color & 0x00FF00) >> 8,
                                              color & 0x0000FF         );
    ...
    protected function drawText(text, size, position, font, color, alignment = 'center', orientation = 0)
    {
        ...
        let position[0] += this->leftOffset;
        let position[1] += this->topOffset;
        ...
        if is_numeric(font) {
            ...
            let fontWidth = imagefontwidth(font);
            let positionY = position[1] - imagefontheight(font) + 1;
            switch alignment {
                case 'left':
                        let positionX = position[0];
                    break;
                case 'center':
                        let positionX = position[0] - ceil((fontWidth * strlen(text)) / 2);
                    break;
                case 'right':
                        let positionX = position[0] - (fontWidth * strlen(text));
                    break;
            }
            ...
            let box = imagettfbbox(size, 0, font, text);
            switch alignment {
                case 'left':
                        let width = 0;
                    break;
                case 'center':
                        let width = (box[2] - box[0]) / 2;
                    break;
                case 'right':
