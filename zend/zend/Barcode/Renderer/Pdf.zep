...
class Pdf extends AbstractRenderer
{
    ...
    {
        let this->resource = pdf;
        let this->page     = intval(page);

        if !count(this->resource->pages) {
            let this->page = 0;
            let this->resource->pages[] = new Page(
                Page::SIZE_A4
            );
        }
        return this;
    ...
    protected function initRenderer()
    {
        if this->resource === null {
            let this->resource = new PdfDocument();
            let this->resource->pages[] = new Page(
                Page::SIZE_A4
            );
        }

        let pdfPage = this->resource->pages[this->page];
    ...
    {
        let page = this->resource->pages[this->page];
        for point in points {
            let x[] = point[0] * this->moduleSize + this->leftOffset;
            let y[] = page->getHeight() - point[1] * this->moduleSize - this->topOffset;
        }
        if count(y) == 4 {
            if x[0] != x[3] && y[0] == y[3] {
                let y[0] -= (this->moduleSize / 2);
                let y[3] -= (this->moduleSize / 2);
            }
            if x[1] != x[2] && y[1] == y[2] {
                let y[1] += (this->moduleSize / 2);
                let y[2] += (this->moduleSize / 2);
            }
        }
    ...
    protected function drawText(
        ...
        let width = this->widthForStringUsingFontSize(
            text,
            Font::fontWithPath(font),
            size * this->moduleSize
        );

        let angle = pi() * orientation / 180;
        let left = position[0] * this->moduleSize + this->leftOffset;
        let top  = page->getHeight() - position[1] * this->moduleSize - this->topOffset;

        switch alignment {
            case 'center':
                    let left -= (width / 2) * cos(angle);
                    let top  -= (width / 2) * sin(angle);
                break;
            case 'right':
                    let left -= width;
                break;
    ...
    {
        let drawingString = iconv('UTF-8', 'UTF-16BE//IGNORE', text);
        let characters    = [];
        var i = 0;
        var len = strlen(drawingString);
        while i < len {
            let characters[] = (ord(drawingString[i ++]) << 8) | ord(drawingString[i]);
            let i++;
        }
        let glyphs = font->glyphNumbersForCharacters(characters);
        let widths = font->widthsForGlyphs(glyphs);
        let stringWidth = (array_sum(widths) / font->getUnitsPerEm()) * fontSize;
        return stringWidth;
    }
}
