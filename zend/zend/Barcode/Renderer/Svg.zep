...
class Svg extends AbstractRenderer
{
    ...
    public function setHeight(value)
    {
        ...
        let this->userHeight = intval(value);
        return this;
    }
    ...
    public function setWidth(value)
    {
        ...
        let this->userWidth = intval(value);
        return this;
    }
    ...
    {
        let barcodeWidth  = this->barcode->getWidth(true);
        let barcodeHeight = this->barcode->getHeight(true);

        let backgroundColor = this->barcode->getBackgroundColor();
        let imageBackgroundColor = 'rgb(' . implode(', ', array((backgroundColor & 0xFF0000) >> 16,
        ...
        if this->resource === null {
            ...
            let this->resource = new DOMDocument('1.0', 'utf-8');
            let this->resource->formatOutput = true;
            let this->rootElement = this->resource->createElement('svg');
            let this->rootElement->setAttribute('xmlns', "http://www.w3.org/2000/svg");
            let this->rootElement->setAttribute('version', '1.1');
            let this->rootElement->setAttribute('width', width);
            let this->rootElement->setAttribute('height', height);
            ...
            let width = this->rootElement->getAttribute('width');
            let height = this->rootElement->getAttribute('height');
        }
    ...
    {
        if this->resource !== null {
            let this->rootElement = this->resource->documentElement;
        }
    }
    ...
    {
        let newElement = this->createElement(tagName, attributes, textContent);
    ...
    {
        let element = this->resource->createElement(tagName);
    ...
    protected function checkDimensions()
    {
        if this->resource !== null {
            this->readRootElement();
            let height = (float) this->rootElement->getAttribute('height');
            ...
            if this->userHeight {
                let height = this->barcode->getHeight(true);
        ...
        if this->resource !== null {
            ...
            if this->userWidth {
                let width = (float) this->barcode->getWidth(true);
    ...
    {
        let color = 'rgb(' . implode(', ', [(color & 0xFF0000) >> 16,
                                              (color & 0x00FF00) >> 8,
                                              (color & 0x0000FF)]) . ')';
        let orientation = this->getBarcode()->getOrientation();
        let newPoints = [
        ...
        let newPoints = implode(' ', newPoints);
        let attributes['points'] = newPoints;
        let attributes['fill'] = color;
    ...
    protected function drawText(text, size, position, font, color, alignment = 'center', orientation = 0)
    {
        let color = 'rgb(' . implode(', ', [(color & 0xFF0000) >> 16,
                                              (color & 0x00FF00) >> 8,
                                              (color & 0x0000FF)]) . ')';
        let attributes['x'] = position[0] + this->leftOffset;
        let attributes['y'] = position[1] + this->topOffset;
        //attributes['font-family'] = font;
        let attributes['color'] = color;
        let attributes['font-size'] = size * 1.2;
        switch alignment {
            case 'left':
        ...
        let attributes['style'] = 'text-anchor: ' . textAnchor;
        let attributes['transform'] = 'rotate('
                                 . (- orientation)
                                 . ', '
